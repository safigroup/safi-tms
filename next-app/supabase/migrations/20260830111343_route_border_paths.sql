-- Optional alternate border-crossing paths for a route. routes.borders
-- keeps meaning exactly what it means today (a route's default path) --
-- this table only holds additional alternatives, so a route with none
-- behaves exactly as before. No org_id FK constraint mismatch risk here:
-- route_id already implies the org via routes.org_id, but org_id is kept
-- as its own column (matching every other table in this schema) so
-- queries can filter directly without an extra join.
create table "public"."route_border_paths" (
  ...
  constraint "route_border_paths_pkey" primary key (id)
);

alter table "public"."route_border_paths"
  enable row level security;

alter table "public"."route_border_paths"
  add constraint "route_border_paths_org_id_fkey" foreign key (org_id) references public.organizations(id) on delete cascade;

alter table "public"."route_border_paths"
  add constraint "route_border_paths_route_id_fkey" foreign key (route_id) references public.routes(id) on delete cascade;

create index route_border_paths_org_id_route_id_idx on public.route_border_paths using btree (org_id, route_id);

grant delete, insert, maintain, references, select, trigger, truncate, update on table "public"."route_border_paths" to "anon", "authenticated", "postgres", "service_role";

-- A trip snapshots whichever path it actually uses (default or a chosen
-- alternate) at creation time, matching this app's existing pattern of
-- freezing a value at the moment it's decided (e.g. FX rates on
-- trip_costs) rather than always live-joining to a source that could
-- change later. Null means "the route's default", same as before this
-- column existed.
alter table "public"."trips"
  add column "borders" text[];

-- trip_board's borders becomes coalesce(trip's own, route's default) so
-- existing trips (no snapshot yet) are unaffected. Everything else
-- identical to the view as it stood in 20260825182600_truck_costs.sql.
...
-- create_trip() gets one new optional parameter. CREATE OR REPLACE does
-- NOT replace a function whose argument count changes -- Postgres treats
-- it as a distinct overload, which caused a real production incident
-- earlier in this project's history (PGRST203 ambiguous-function errors).
-- The old 13-arg signature must be dropped explicitly.
